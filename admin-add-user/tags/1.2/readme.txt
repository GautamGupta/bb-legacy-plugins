=== Admin add user ===
Tags: admin, keymaster, registration, user
Contributors: thomasklaiber
Requires at least: 0.74
Tested up to: 1.0-alpha
Stable Tag: 1.2

Allows the Keymaster to add users through the Administration panel.

== Description ==

This plugin adds a new Function called `Add User` to the Administration. There the keymaster is able to add new users to the board.

== Installation ==

Add `admin-add-user.php` to your `/my-plugins/` directory.

== Configuration ==

not needed.

== Frequently Asked Questions ==

= I'm using bbpress & Wordpress together? =

So then, you don't really need this plugin, because Wordpress already has this feature!