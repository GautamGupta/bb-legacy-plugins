...
function automated_forum_moderation_initial_blocking($retvalue, $capability, $args) {
	...
	if ($capability == 'write_post') {
		if ((!$automated_forum_moderation_data['allow_double_post'] && $bb_post->poster_id === bb_get_current_user_info('id') && ((bb_current_time('timestamp') - bb_gmtstrtotime($bb_post->post_time))/60) < $automated_forum_moderation_data['allow_double_post_after']) || // Double posting is not allowed
			...
			intval((bb_current_time('timestamp') - bb_gmtstrtotime($bb_post->post_time))/86400) > $automated_forum_moderation_data['max_days'] && !bb_current_user_can('close_topic', get_topic_id())) // Topic is old
				return false;
	}
	return $retvalue;
}
...
<?php } elseif (!empty($bb_post) && intval((bb_current_time('timestamp') - bb_gmtstrtotime($bb_post->post_time))/86400) > 30 && bb_current_user_can('close_topic', get_topic_id())) { ?>
	<p>This topic is old. It has been automatically closed to new replies. However, if you feel there is a need for this topic to be re-opened, you can post here.</p>
...
function automated_forum_moderation_jit_blocking($post_text, $post_id = 'error', $topic_id = 'error') { // JIT = Just In Time
	...
	if ($last_post_in_topic !== null) {
		...
		if (intval((bb_current_time('timestamp') - bb_gmtstrtotime($last_post_time))/86400) > $automated_forum_moderation_data['max_days'] && !bb_current_user_can('close_topic', $topic_id)) {
		...
		// We want ro really delete the topic since it has no reason to be in the database (it has no posts and none can be added)
