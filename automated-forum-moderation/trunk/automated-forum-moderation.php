...
function automated_forum_moderation_jit_blocking($post_text, $post_id, $topic_id) { // JIT = Just In Time
	...
	$last_post_in_topic = $bbdb->get_row($bbdb->prepare('SELECT `poster_id`, `post_time`, `forum_id` FROM `'.$bbdb->posts.'` WHERE `topic_id` = %d AND `post_status` = \'0\' LIMIT 1 ORDER `post_time` DESC, `post_position` ASC', $topic_id)); // Only get what we need
	if ($last_post_in_topic->post_time) {
	...
	if (count(explode(' ', $post_text)) < $automated_forum_moderation_data['min_words']) {
		if ($is_new_topic) {
			error_reporting(E_ALL ^ (E_WARNING & E_NOTICE)); // Hack to remove php error caused by no posts being in the topic yet.
