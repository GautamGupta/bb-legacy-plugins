...
Description: This plugin generates persistent specific geometric icons for each user based on the ideas of <a href="http://www.docuverse.com/blog/donpark/2007/01/18/visual-security-9-block-ip-identification">Don Park</a>. Contains Identicon class only now - fel.
...
function felapplyidenticon( $felID )
{
	...
	if( $identicon ) {
		...
		if( imagepng( $felidenticon, $ifilepath ) ) {
			...
			$meta_avatar = $ifilename . '|' . $ioptions['size'] . '|' . $ioptions['size'] . '|identicon';
...
add_action( 'bb_new_user', 'felapplyidenticon' );
