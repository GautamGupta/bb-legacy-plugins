...
?>
<div class="wrap">
	<h2>BBpress Latest Discussions</h2>
	<?php if ($update_msg) { echo $update_msg; } ?>
	<p><strong>Plugin Name:</strong> BBpress Latest Discussions<br />
	<strong>Plugin URI:</strong> <a href="http://www.atsutane.net/2006/11/bbpress-latest-discussion-for-wordpress/">http://www.atsutane.net/2006/11/bbpress-latest-discussion-for-wordpress/</a><br />
	<strong>Author:</strong> <a href="http://www.atsutane.net/">Atsutane Shirane</a><br />
	<strong>Version:</strong> 0.7</p>
	<p><strong>ToDo List:</strong></p>
	<ul>
		<li>Add option to exclude some forum.</li>
	</ul>
	<p>If you have any suggestion or feedback. Feel free to post it <a href="http://www.atsutane.net/2006/11/bbpress-latest-discussion-for-wordpress/">here</a>.</p>
	<h2>BBpress Option</h2>
	<form method="post" action="<?php echo $ori_url; ?>">
		<p class="submit">
			<input type="submit" name="wpbb_save" id="wpbb_save" value="Update Option &raquo;">
		</p>
		<table class="optiontable"> 
			<tr valign="top"> 
				<th scope="row">Bbpress URL:</th> 
				<td><input name="bburl" type="text" id="bburl" value="<?php echo get_option('wpbb_path'); ?>" size="40" /></td> 
			</tr> 
			<tr valign="top"> 
				<th scope="row">Post Limit:</th> 
				<td><input name="bblimit" type="text" id="bblimit" value="<?php echo get_option('wpbb_limit'); ?>" size="3" /> posts</td>
			</tr>
			<tr valign="top"> 
				<th scope="row">Bbpress DB Prefix:</th> 
				<td><input name="bbprefix" type="text" id="bbprefix" value="<?php echo get_option('wpbb_bbprefix'); ?>" size="3" /> Bbpress table prefix<br />Enter the table prefix for your bbPress installation above. The table prefix is found in your bbPress installation's config.php file.</td>
			</tr>
			<tr valign="top">
				<th scope="row">Bbpress Permalink:</th>
				<td><label for="wpbb_permalink"><input name="wpbb_permalink" type="checkbox" id="wpbb_permalink" value="wpbb_permalink" <?php if (get_option('wpbb_permalink')) { echo('checked="checked"'); } ?> /> Use Bbpress Permalink</label><br />Only use this if you already enable permalink inside Bbpress config.php</td>
			</tr>
			<tr valign="top">
				<th scope="row">Wordpress/Bbpress Integration:</th>
				<td><label for="wpbb_intergrated"><input name="wpbb_intergrated" type="checkbox" id="wpbb_intergrated" value="wpbb_intergrated" <?php if (get_option('wpbb_intergrated')) { echo('checked="checked"'); } ?> /> Intergrated with Wordpress</label><br />Check this option if you intergrated your wordpress installation with bbpress installation.</td>
			</tr>
			<tr valign="top">
				<th scope="row">External DB:</th>
				<td><label for="use_outdb"><input name="use_outdb" type="checkbox" id="use_outdb" value="use_outdb" <?php if (get_option('wpbb_exdb')) { echo('checked="checked"'); } ?> /> Use Different DB</label><br />Only enable this option if your Bbpress database is not the same as Wordpress database.</td>
			</tr>
			<tr valign="top"> 
				<th scope="row">Bbpress DB User:</th> 
				<td><input name="bbuser" type="text" id="bbuser" value="<?php echo get_option('wpbb_dbuser'); ?>" size="40" />
			</tr>
			<tr valign="top"> 
				<th scope="row">Bbpress DB Pass:</th> 
				<td><input name="bbpass" type="text" id="bbpass" value="<?php echo get_option('wpbb_dbpass'); ?>" size="40" />
			</tr>
			<tr valign="top"> 
				<th scope="row">Bbpress DB Name:</th> 
				<td><input name="bbname" type="text" id="bbname" value="<?php echo get_option('wpbb_dbname'); ?>" size="40" />
			</tr>
			<tr valign="top"> 
				<th scope="row">Bbpress DB Host:</th> 
	...
	if ($bbtopic) {
		...
		foreach ( $bbtopic as $bbtopic ) {
			...
			if (get_option('wpbb_permalink')) {
				echo "<td><a href=\"$wpbburl/topic/$bbtopic->topic_id\">$bbtopic->topic_title</a></td>";
			}
			else {
				echo "<td><a href=\"$wpbburl/topic.php?id=$bbtopic->topic_id\">$bbtopic->topic_title</a></td>";
			}
			echo "
					<td class=\"num\">$bbtopic->topic_posts</td>
			";
			if (get_option('wpbb_intergrated')) {
				...
				if ($wpuid) {
					...
					echo "<td class=\"num\">$user_forum_data->display_name</td>";
				}
				else {
					echo "<td class=\"num\">$bbtopic->topic_last_poster_name</td>";
				}
			}
			else {
				echo "<td class=\"num\">$bbtopic->topic_last_poster_name</td>";
...
function wp_bb_get_discuss_sidebar() {
	...
	if ($bbtopic) {
		echo "
			<h2>Last $forum_slimit Discussions</h2>
			<ul>
		";
		foreach ( $bbtopic as $bbtopic ) {
			if (get_option('wpbb_permalink')) {
				echo "<li><a href=\"$wpbburl/topic/$bbtopic->topic_id\">$bbtopic->topic_title</a><br />";
			}
			else {
				echo "<li><a href=\"$wpbburl/topic.php?id=$bbtopic->topic_id\">$bbtopic->topic_title</a><br />";
			}
			if (get_option('wpbb_intergrated')) {
				$wpuid = $wpdb->get_row("SELECT * FROM $wordpress_table WHERE user_login = '$bbtopic->topic_last_poster_name'");
				if ($wpuid) {
					$user_forum_data = "$bbtopic->topic_last_poster_name";
					$user_forum_data = get_userdata($wpuid->ID);
					echo "<small>Last Post By: $user_forum_data->display_name</small></li>";
				}
				else {
					echo "<small>Last Post By: $bbtopic->topic_last_poster_name</small></li>";
				}
			}
			else {	
				echo "<small>Last Post By: $bbtopic->topic_last_poster_name</small></li>";
