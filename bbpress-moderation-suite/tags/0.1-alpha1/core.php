...
function bbmodsuite_admin_add() {
	...
	$plugins[] = array(__('bbPress Moderation Suite'), 'use_keys', 'bbpress_moderation_suite');
	...
	if ($plugin && $action && bb_verify_nonce($_GET['_wpnonce'], $action . '-plugin_' . $plugin)) {
		switch ($action) {
			case 'activate':
				...
				bb_admin_notice(sprintf(__('Plugin "%s" <strong>activated</strong>'), $bbmod_plugins[$plugin]['name']));
				break;
			case 'deactivate':
				if (!in_array($plugin, $active_bbmod_plugins) ||
					!isset($bbmod_plugins[$plugin])) break;
				...
				$active_bbmod_plugins = array_flip($active_bbmod_plugins);
				bb_update_option('bbpress_moderation_suite_helpers', $active_bbmod_plugins);
				bb_admin_notice(sprintf(__('Plugin "%s" <strong>deactivated</strong>'), $bbmod_plugins[$plugin]['name']));
			case 'uninstall':
				if (!in_array($plugin, $active_bbmod_plugins) ||
					!isset($bbmod_plugins[$plugin])) break;
				$active_bbmod_plugins = array_flip($active_bbmod_plugins);
				...
				bb_admin_notice(sprintf(__('Plugin "%s" <strong>deactivated</strong> and <strong>uninstalled</strong>'), $bbmod_plugins[$plugin]['name']));
		}
	}
}
...
function bbpress_moderation_suite() {
	global $bbmod_plugins, $active_bbmod_plugins;
?>
<h2><?php _e('bbPress Moderation Suite'); ?></h2>

<table class="widefat">
	<thead>
		<tr>
			<th><?php _e('Moderation Assistants'); ?></th>
