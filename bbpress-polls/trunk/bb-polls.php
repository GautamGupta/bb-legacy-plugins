...
*/
	...
	$bb_polls['style']="#bb_polls {list-style: none; width:350px; margin:auto -10px; line-height:120%; padding:5px; border:1px solid #ADADAD;  font-size:85%; color:#000; background:#eee; }
			#bb_polls .submit {cursor: pointer; cursor: hand; text-align:center;}
...
function bb_polls_add_header() { 
	...
	if ($topic_id=$_GET['show_poll_results_ajax']) {
		echo 'bb_polls_insert_ajax("'.mysql_real_escape_string(bb_polls_show_poll_results($topic_id,0)).'")';
		exit();
	}
	if ($topic_id=$_GET['show_poll_vote_form_ajax']) {
		echo 'bb_polls_insert_ajax("'.mysql_real_escape_string(bb_polls_show_poll_vote_form($topic_id,0)).'")';
		exit();
	}
