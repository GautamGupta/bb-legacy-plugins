...
if ( ! isset($topic->poll_options)) {	// no saved poll question with options
...
remove_action('topicmeta','bb_polls_pre_poll');  // NullFix ?
} add_action('topicmeta','bb_polls_pre_poll');

function bb_polls_check_cache($topic_id) {
global $bb_polls,$topic,$poll_options;  
if (!$topic_id || $topic_id != $topic->topic_id) {$topic_id=get_topic_id( $topic_id );  $topic = get_topic($topic_id); $poll_options=(isset($topic->poll_options)==true ? $topic->poll_options : null); }
