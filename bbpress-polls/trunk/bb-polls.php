...
if (!$poll_options['poll_multiple_choice']) {$real_vote_count=intval($poll_options['poll_count_0']);}
else {for ($i=1; $i<=$bb_polls['max_options']; $i++) {$real_vote_count=$real_vote_count+intval($poll_options['poll_count_'.$i]);}}
...
$output=stripslashes($output);if ($display) {echo '<li id="bb_polls">'.$output.'</li>';} else {return $output;}
}
}
...
function bb_polls_insert_ajax(htmldata) {
	document.getElementById("bb_polls").innerHTML=unescape(htmldata);
}
...
var bb_polls_poll_setup_form = "'.mysql_escape_string(bb_polls_show_poll_setup_form(get_topic_id( $topic_id ),0)).'";
...
var bb_polls_script3 = document.createElement("script");
bb_polls_script3.src = "'.add_query_arg( 'add_vote_ajax', get_topic_id( $topic_id )).'"+vote;
bb_polls_script3.type = "text/javascript";
bb_polls_script3.charset = "utf-8";
document.getElementsByTagName("head")[0].appendChild(bb_polls_script3);
return true;
}
</scr'.'ipt>';
endif;
