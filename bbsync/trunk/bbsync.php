...
load replies if single post, forget felbbreplies()
*/
...
function felsyncpost( $post_id ) {
	global $bbdb, $wpdb, $current_user, $bb_current_user, $bb_cache, $bb_roles;
...
/**/	stat_bbnocaps();
	...
	// fix for ensuing problems with bb permissions found and implemented by jazbek, initiated by SlexAxton. Thank you very much!
	$bb_current_user = $current_user;
	
	$post_title = $bbdb->escape( $post->post_title );
	$post_text = $bbdb->escape( felprepposttext( $post_id ) );
...
function zombie_bbnocaps() { add_action('bb_user_has_no_caps', 'kill_nobbcaps', 1 ); }
function kill_bbnocaps() { remove_action('bb_user_has_no_caps', 'bb_give_user_default_role'); }
function stat_bbnocaps() { add_action('bb_user_has_no_caps', 'bb_give_user_default_role'); }
...
function felbringtospeed() {
	//post links only in bb_topicmeta now also plz in wp_postmeta
	global $bbdb;
	$opshuns = get_option('bbsync');
	require_once $opshuns['bbpath'];
	$topiclinks = $bbdb->get_results("SELECT topic_id, meta_value FROM $bbdb->topicmeta WHERE meta_key = 'wp_post'");
	foreach( $topiclinks AS $link ) {
		felmetalink( $link->meta_value, $link->topic_id );
	}
}
...
<div class="wrap">
	...
	<div>
		<h3>Update Plugin</h3>
		<p>This will get all the old links and update them to enable caching and stuff. Do it if you just replaced this plugin with a newer version.</p>
		<form method="POST" action="<?php echo $bbsyncurl; ?>">
			<input type="hidden" name="bringtospeed" value="true" />
