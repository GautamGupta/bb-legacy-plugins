...
Instructions:   This is an auto-load plugin and must have a leading underscore "_". 
Put into your plugin directory, access bbPress and/or WordPress once and then delete it from the directory.

Warning: This plugin assumes your keymaster is user id #1. 
It is rare but possible you are no longer user #1 if you have deleted your original keymaster. In which case this won't work.
*/

function fix_admin_access() {

$id=1;	 	//  assume keymaster was user #1, you can change this to another number if you are positive it's different
...
foreach ($meta as $key => $value) :
	if ($db->get_var("SELECT meta_value FROM $db->usermeta WHERE meta_key = '$key' AND user_id = '$id' LIMIT 1"))
	            {$db->query("UPDATE $db->usermeta SET meta_value = '$value' WHERE user_id = '$id' AND meta_key = '$key' LIMIT 1");}
     	   else {$db->query("INSERT INTO $db->usermeta  (user_id, meta_key, meta_value)  VALUES ('$id', '$key', '$value')");}
endforeach;
} 
}
