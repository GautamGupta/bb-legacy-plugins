...
function get_hot_tags_plus($args = '' ) {
	...
	$query="SELECT COUNT( relationships.term_taxonomy_id ) AS tcount, name AS raw_tag, slug AS tag
	...
	$query="SELECT COUNT(relationships.tag_id) as tcount, terms.raw_tag AS raw_tag, terms.tag AS tag
	...
	foreach ( $counts as $tag => $count ) {
		...
		$a[] = "<a href='$taglink' title='" . attribute_escape( sprintf( __('%d topics'), $count ) ) . "' rel='tag'><span style='font-size: " .
