...
optionally write questions in captcha-like graphic (tricks spammers to enter graphic as code instead of answer)
...
if (human_register_page()) :  //  if (bb_current_user_can( 'administrate' ) )  :
	...
	echo '<fieldset><legend>Please prove you are human</legend><table width="100%"><tr class="required"><th scope="row"  width=199>';
	...
	if ($_POST) {
		if  (isset($_POST['human_test'])) {
			...
			if ($human_test !=$_SESSION['HUMAN_TEST']) {
				...
				bb_die(__("Humans only please. If you are not a bot, please go back and try again."));				
			}	
		}
	} else {
	@session_start();
