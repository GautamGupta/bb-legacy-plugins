...
global $leaders,$page,$total;  $days=isset($_REQUEST['days']) ? intval($_REQUEST['days']) : 0;
$output='
<h3 class="bbcrumb"><a href="'.bb_get_option('uri').'">'.bb_get_option('name').'</a> &raquo; Leaderboard</h3>

<div class="indent">

<h2>Leaderboard</h2>

<form class="days" method="get" action="'.preg_replace( '|\/page\/[0-9]+|', '/', $_SERVER["REQUEST_URI"]).'">

	<ul class="leaderboard">
	<li>View most active contributors: </li>	
';
// $output.='<li>in forum '.leaderboard_forum_list().'<br /></li>';
$output.='
	<li><input id="leadersall" name="days" value="0" onclick="this.form.submit()" type="radio" '.(!$days ? 'checked="checked"' : '').' <label for="leadersall">Of All-Time</label></li>
	<li><input id="leaders30" name="days" value="30" onclick="this.form.submit()" type="radio" '.($days==30 ? 'checked="checked"' : '').' <label for="leaders30">From The Past Month</label></li>
	<li><input id="leaders7" name="days" value="7" onclick="this.form.submit()" type="radio"  '.($days==7 ? 'checked="checked"' : '').' <label for="leaders7">From The Past Week</label></li>
	<li><input id="leaders1"name="days" value="1" onclick="this.form.submit()" type="radio" '.($days==1 ? 'checked="checked"' : '').' <label for="leaders1">From Today</label></li>
	</ul>	
</form>
';	
	if (empty($leaders)) {
...
$output.='<div class="nav">'.get_page_number_links( $page, $total ).'</div></div>';
