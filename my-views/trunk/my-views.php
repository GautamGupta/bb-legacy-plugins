...
Description:  
1. adds views for a user's "topics started" and other "participated topics"
2. also adds "most views" and "least views" if bb-topic-views is installed. 
3. also adds page titles to views which are missing by bbpress default. 
4. optional dropdown views list  anywhere you put <? views_dropdown(); ?> in templates.
5. also adds forum view count if bb-topic-views is installed. Edit front-page.php template to show <? echo $forum->views; ?>; in desired column.
...
Instructions:
1. Install, activate 
2. optionally install bb-topic-views plugin for view counts and edit your view.php template for view count / dropdown view list
3. optionally edit front-page.php to show view count for each forum in table ie. $forum->views;
...
$my_views_prefered_order=array(
	"latest-discussions","no-replies","untagged","my-topics","my-posts","most-views","least-views","installed-plugins","available-plugins"
	);
...
function my_views_dropdown($display=1) {	/*  makes views available as dropdown list anywhere you put <? my_views_dropdown(); ?> */
	...
	$views=get_views(); 
	foreach ($views as $view => $title ) {$views_dropdown.='<option value="'.get_view_link($view).'">'.$views[$view].'</option>';}
	$views_dropdown.='</select></form>'; 
if ($display) {echo $views_dropdown;} else {return $views_dropdown;}
}

function my_views_header($bbcrumb=false) {	/*  adds proper h2 header & dropdown to view.php template(s)  put <? my_views_header(); ?> */	
if (is_view()) : 
if ($bbcrumb) :	?>
<h3 class="bbcrumb"><a href="<?php bb_option('uri'); ?>"><?php bb_option('name'); ?></a> &raquo; <?php view_name(); ?></h3>
<?php 	endif;	?>
<h2 style="float:left;width:50%;"><?php view_name(); ?></h2><div style="float:right"><?php my_views_dropdown(); ?></div>
<br clear=both>
<?
endif;
}
