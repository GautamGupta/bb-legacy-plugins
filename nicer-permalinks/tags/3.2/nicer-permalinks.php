...
function nicer_get_post_link_filter ($link, $post_id = 0, $topic_id = 0) {
	...
	if ($post_page>1) $topic_uri .= "/page/$post_page"; // append post page to topic URI, if needed

	// build nicer post URI emphasizing hierarchy
	return bb_get_option('uri')."$forum->forum_slug/$topic->topic_slug#post-".get_post_id($post_id);
}
