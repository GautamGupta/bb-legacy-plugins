...
function nicer_permalinks_process_config_page() {
	if ( NICER_PERMALINKS_ID == $_POST['referrer'] ) {
		...
		if ( $requested_status != $status ) { // Process request if requested status does not match plugin current status
			...
			if ( $status ) { // Try to disable the plugin
...
function nicer_permalinks_config_errors( $status ) {
	...
	if ( 0 == $status ) { // Check if plugin can be enabled
	...
	} elseif ( 1 == $status ) { // Check if plugin can be disabled
		if ( !is_writable( BB_PATH . '.htaccess' ) ) // Generate error if .htaccess is not writable
			$errors[] = sprintf( __( '<code>%s</code> is not writable.', NICER_PERMALINKS_ID ), '.htaccess' );
	} else { // Generate error if $status does not match any previous case (should never happen)
		$errors[] = sprintf( __( '%s is not a valid plugin status.', NICER_PERMALINKS_ID ), $status );
	}
...
/**
 * PHP4 backwards compatibility functions
 *
 * Note: the following functions are NOT solid, please do NOT use them anywhere else!
 */
