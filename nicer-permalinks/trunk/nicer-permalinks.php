...
function nicer_get_forum_link_filter( $link, $forum_id = 0 ) {
	...
	$link = str_replace( $bb_uri .'forum/', $bb_uri, $link );
...
function nicer_get_topic_link_filter( $link, $id = 0 ) {
	...
	if ( !$topic ) // deleted topic redirection link request
		return $bb_uri; // redirect to main page because no topic info can be retrieved (bug fixed in bbPress 1.1-alpha)
...
function nicer_get_post_link_filter( $link, $post_id = 0, $topic_id = 0 ) {
	...
	return $bb_uri ."$forum->forum_slug/$topic->topic_slug$page#post-". get_post_id($post_id);
}
