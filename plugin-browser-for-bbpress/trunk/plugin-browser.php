...
/**
 * Plugin browser for bbPress version 0.1.4
 * 
 * ----------------------------------------------------------------------------------
 * 
...
class Plugin_Browser
{
		...
		if (!is_file(BBPLUGINDIR . 'pb--' . $plugin_id)) {
			if (mkdir(BBPLUGINDIR . 'pb--' . $plugin_id)) {
				...
				foreach ($list as $item) {
					$file = $this->getRemoteFile($this->repositoryURI . $plugin_id . '/trunk/' . $item);
					
					$handle = fopen(BBPLUGINDIR . 'pb--' . $plugin_id . '/' . $item, 'w');
	...
	function uninstallPlugin($plugin_id)
	{
		if (is_dir(BBPLUGINDIR . 'pb--' . $plugin_id . '/')) {
			foreach (glob(BBPLUGINDIR . 'pb--' . $plugin_id . '/*') as $file) {
				unlink($file);
			}
			if (!rmdir(BBPLUGINDIR . 'pb--' . $plugin_id)) {
				return false;
			}
		}
