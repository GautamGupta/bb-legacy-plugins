...
function pm_post_form( $h2 = '' ) {
	...
	if ( bb_current_user_can( 'write_post' ) ) {
		...
		if ( file_exists( BBPATH . 'my-templates/pm-form.php' ) ) {
			include( BBPATH . 'my-templates/pm-form.php' );
		} else {
			die("You do not have pm-form.php in my-templates");
		}
		echo "\n</form>";
...
function pm_reply_form( $h2 = '', $pmmessage ) {
	...
	if ( !bb_is_user_logged_in() ) {
		...
		if ( file_exists( BBPATH . 'my-templates/message-form.php' ) ) {
			include( BBPATH . 'my-templates/message-form.php' );
		} else {
			die("You do not have message-form.php in my-templates");
		}
		echo "\n</form>"; 
	}
}
...
function pm_user_form( $pmuser, $h2 = '') {
	...
	if ( bb_current_user_can( 'write_post' ) ) {
		echo "<form class='postform' name='postform' id='postform' method='post' action='" . bb_get_option('uri') . "pm-post.php'>\n";
		if ( file_exists( BBPATH . 'my-templates/pm-user-form.php' ) ) {
			include( BBPATH . 'my-templates/pm-user-form.php' );
		} else {
			die("You do not have pm-user-form.php in my-templates");
		}
		echo "\n</form>";
	} elseif ( !bb_is_user_logged_in() ) {
		echo '<p>';
