...
define( 'ACTIVE_TEMPLATE_IMAGES_URI',        bb_get_active_theme_uri() . 'images/' );
define( 'VIEWS_IGNORE_STICKIES_PRIORITY',    false ); // Set this to true to make support views override stickies priority order
...
class Support_Forums_Settings {
	...
	function isStatusChangeable() {
		...
		return (bool) apply_filters( 'poster_changeable', $this->posterChangeable, $topic->topic_id );
	}
	...
	function supportTopicMeta() {
		...
		if ( true === (bool) $this->areIconsEnabled() ) { // Add closed and sticky statements if icons are enabled
			$icons_uri = ( array_key_exists( 'dir', $this->icons ) ) ? ACTIVE_TEMPLATE_IMAGES_URI . $this->icons['dir'] : SUPPORT_FORUMS_ICONS_URI;
...
EOF;
	 ...
	 * @return boolean
	 */
	function setTopicStatus( $topic_id = 0, $status = 'resolved' ) {
		if ( false === (bool) $this->isEnabled() )
			return false;

		// Apply filters, if any
		apply_filters( 'support_forums_change_topic_status', $topic_id, $status );
	...
	function registerSupportViews() {
		...
		foreach ( $this->views as $view ) {
			...
			$query = array(
				'sticky' =>    ( true === (bool) VIEWS_IGNORE_STICKIES_PRIORITY ) ? 'all' : NULL,
