...
function swirl_unknowns_config_page() {
	...
	$swirl_page = bb_get_option( 'swirl_page' );

	if ( is_null( $swirl_page ) ) // Set var to empty string if it did not exist
		$swirl_page = '';
...
<fieldset class="submit">
	<input type="hidden" name="action" id="action" value="<?php echo SWIRL_UNKNOWNS_ID ?>" />
...
function swirl_unknowns_process_config_page() {
	if ( SWIRL_UNKNOWNS_ID == $_POST['action'] ) {
		global $notices;
