...
add_action( 'bb_admin_head','tag_history_header',100); 
function tag_history_per_page($per_page) {return 100;}

add_action( 'bb_get_option_page_topics', 'tag_history_per_page',250);
function tag_history_header() {echo '<style type="text/css">table.widefat th, table.widefat td {text-align:left;} .timetitle {cursor:help;} .page-numbers {margin:0 2px;}</style>';}
...
elseif (isset($_GET['tag_id'])) {$tag_id=intval($_GET['tag_id']);}
elseif (isset($_GET['topic_id'])) {$topic_id=intval($_GET['topic_id']);}
if ($tag_id || $user_id || $topic_id) {	
	$reset=' &nbsp; [<a href="'.$base.'">RESET</a>]';	
	$where='WHERE '.($tag_id ? " tag_id=$tag_id " : '').($user_id ? " user_id=$user_id " : '').($topic_id ? " topic_id=$topic_id " : '');
}
...
$temp=$bbdb->get_results("SELECT user_id,count(*) as tag_count FROM $bbdb->tagged WHERE user_id IN (".implode(',',$users).") GROUP BY user_id");
foreach ($temp as $item) {$user_count[$item->user_id]=$item->tag_count;}
bb_cache_post_topics($topics);
$temp=$bbdb->get_results("SELECT * FROM $bbdb->tags WHERE tag_id IN (".implode(',',$tags).")");   // tag_id tag raw_tag tag_count
unset($tags); foreach ($temp as $item) {$tags[$item->tag_id]=$item;}		

if ($tag_id) {$for=" for tag <span class='alt'><i>".$tags[$tag_id]->raw_tag."</i></span> ";}
elseif ($user_id) {$for=" for user <span class='alt'><i>".get_user_name($user_id)."</i></span> ";}
...
echo '<table class="widefat"><thead><tr><th>Tag</th><th>By</th><th>Topic</th><th>Date</th></tr></thead>';
