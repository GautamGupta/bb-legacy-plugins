...
Description: topic/forum visibility
Version: 0.1.2
Author: Francesco Bigiarini
Author URI: http://www.zaerl.com

zaerl Visibility: topic/forum visibility for bbPress
...
define('ZA_VISIBILITY_VERSION', '0.1.2');
define('ZA_VI_ID', 'za-visibility');
define('ZA_VISIBILITY_NAME', 'zaerl Visibility');
...
function za_vi_topic_has_visibility($id)
{
	global $za_vi_settings;
...
function za_vi_initialize()
{
	...
	global $za_vi_settings, $za_vi_hidden_forums_list, $za_vi_hidden_topics_list,
		$za_vi_locked_forums_count, $za_vi_hidden_topics_count,
		$za_vi_current_user_role, $bb_views,
		$bb_current_user, $forum_id, $page;
	...
	za_vi_var(array('su', 'sr', 'fhr', 'fhu', 'fehu', 'flr', 'flu', 'felu',
		'thr', 'thu', 'tehu', 'tlr', 'tlu', 'telu', 'fp', 'tp', 'flm', 'tlm',
		'tlm'));
	...
	{	
		...
		{
			foreach($za_vi_settings['fhr'] as $key => $value) // forum hide role
				if(in_array($za_vi_current_user_role, $value)) $za_vi_hidden_forums_list[] = $key;
			
			if($id != 0 && !empty($za_vi_settings['fhu'])) // forum hide user
			{
			...
			if($id != 0 && $ehu_count > 0 && !empty($za_vi_hidden_forums_list)) // forum hide user exception
			{
			...
			foreach($za_vi_settings['thr'] as $key => $value) // topic hide role
				if(in_array($za_vi_current_user_role, $value)) $za_vi_hidden_topics_list[] = $key;

			if($id != 0 && !empty($za_vi_settings['thu'])) // topic hide user
			{
			...
			if($id != 0 && $ehu_count > 0 && !empty($za_vi_hidden_topics_list)) // topic hide user exception
			{
				...
				!empty($za_vi_settings['tlu']))
			{
				...
				add_action('bb_forum.php', 'za_vi_template_hook'); // site.com/forum/forum-name
				add_action('bb_topic.php', 'za_vi_template_hook'); // site.com/topic/topic-name
				add_action('bb_front-page.php', 'za_vi_template_hook'); // site.com/
			}
		}
	}
...
function za_vi_get_label($label, $forum_id, $topic_id)
{
	...
	/*$hr = $topic_id != '' ? $za_vi_settings['thr'][$topic_id] :
		$za_vi_settings['fhr'][$forum_id];

	$lr = $topic_id != '' ? $za_vi_settings['tlr'][$topic_id] :
		$za_vi_settings['flr'][$forum_id];


	if(stripos($label, '%hide_roles%') !== FALSE)
	{
		$label = str_ireplace('%hide_roles%', !empty($hr) ? implode(',', $hr) : '',
			$label);
	}
	
	if(stripos($label, '%lock_roles%') !== FALSE)
	{
		$label = str_ireplace('%lock_roles%', !empty($lr) ? implode(',', $lr) : '',
			$label);
	}*/
...
function za_vi_hide_link($args = '')
{
	...
	$ret = __('Modify Hide/Lock Status', ZA_VI_ID);
...
?>
<h2><?php /* Translators: %s is replaced by the program name */ printf(__('%s Settings', ZA_VI_ID), ZA_VISIBILITY_NAME); ?></h2>
...
<?php
	...
	if(isset($_GET['id']) || isset($_GET['fid'])) // Topics/Forums special page
	{
...
?>
	<fieldset>
		...
		} else
		{
			...
			'<a href="' . get_forum_link($topic->forum_id) . '">' . bb_get_forum(get_forum_id($topic->forum_id))->forum_name . '</a>');
		}
...
<?php
		...
		$hu = $is_forum ? $za_vi_settings['fhu'][$id] : $za_vi_settings['thu'][$id];
		...
		$hu = $is_forum ? $za_vi_settings['fehu'][$id] : $za_vi_settings['tehu'][$id];
		$hidden_users_names = array();
...
?>
		<div>
			<div class="label"><?php $is_forum ? _e('Roles Lock Rules', ZA_VI_ID) : _e('Roles Lock Rules', ZA_VI_ID) ?></div>
			<div class="inputs">
<?php

		if($is_forum) $fb = $za_vi_settings['flr'][$id];
		else $fb = $za_vi_settings['tlr'][$id];
		...
		{ ?>
				<label class="checkboxs"><input type="checkbox" class="checkbox" name="<?php
					...
					/*if(!$is_forum)
					{
						if(isset($froles) && in_array($i, $froles)) echo $hidden_suffix, ' ';
						//if(isset($broles) && in_array($i, $broles)) echo $locked_suffix;
					}*/
		...
		$bu = $is_forum ? $za_vi_settings['flu'][$id] : $za_vi_settings['tlu'][$id];
		...
		$bu = $is_forum ? $za_vi_settings['felu'][$id] : $za_vi_settings['telu'][$id];
		$locked_users_names = array();
...
<?php
	...
	<fieldset>
		...
		<p><?php _e('This is a list of all your forums. Please notice that a subforum inherit hide rules from its parent forum.', ZA_VI_ID) ?><p>
		<table class="widefat">
			<thead>
...
?>
	...
	<fieldset>
		...
		{
			 echo '<p>', __('This is a list of those topics that are affected by hide/lock own rules. You can handle the visibility of a topic through the link that you can find in administration menu at the bottom of its page.', ZA_VI_ID), '<p>';
?>
		<table class="widefat">
			<thead>
...
?>
	...
	<fieldset>
		...
		echo "\t\t<p>", __('You can specify a prefix that will be appended to hidden/locked forums/topics titles. <code>%status%</code> will be substituted by a brief string that synthetize the presence of visibility rules ("+hr" = hide rules, "+hu" = hide users, "-hu" = hide users exceptions, "+lr" = lock rules, "+lu" = lock users, "-lu" = lock users exceptions) and parent forum rules, if any, (same syntax but with capital letters.)', ZA_VI_ID),
			"</p>\n";
	...
	<fieldset>
		<legend><?php _e('Replacement Messages', ZA_VI_ID) ?></legend>
...
{
	bb_admin_add_submenu(ZA_VISIBILITY_NAME, 'moderate', 'za_vi_configuration_page', 'options-general.php');
}
...
function za_vi_configuration_page_process()
{
	...
	if('post' == strtolower($_SERVER['REQUEST_METHOD']))
	{
		...
		$goback = remove_query_arg(array('za-visibility-updated', 'za-zero-forums'), wp_get_referer());
		...
		{
			$is_forum = $_POST['action'] == 'update-za-vi-forum-settings';
			...
			$new_lock_roles = array();

			foreach($roles as $i => $v)
			{
				if($_POST["za_vi_hide_$i"] == 'on') $new_hide_roles[] = $i;
				if($_POST["za_vi_lock_$i"] == 'on') $new_lock_roles[] = $i;
			}

			// Save new hide roles
			if(!empty($new_hide_roles))
			{
				$za_vi_settings[$is_forum ? 'fhr' : 'thr'][$id] = $new_hide_roles;
			} else unset($za_vi_settings[$is_forum ? 'fhr' : 'thr'][$id]);

			// Save new hide users			
			$list1 = za_vi_explode_user_list($_POST['za_vi_hidden_users']);
			
			if(!empty($list1)) $za_vi_settings[$is_forum ? 'fhu' : 'thu'][$id] = $list1;
			else unset($za_vi_settings[$is_forum ? 'fhu' : 'thu'][$id]);

			// Save new hide exception users
			$list2 = za_vi_explode_user_list($_POST['za_vi_ex_hidden_users']);
			
			if(!empty($list2)) $za_vi_settings[$is_forum ? 'fehu' : 'tehu'][$id] = $list2;
			else unset($za_vi_settings[$is_forum ? 'fehu' : 'tehu'][$id]);
			...
			{
				$za_vi_settings[$is_forum ? 'flr' : 'tlr'][$id] = $new_lock_roles;
			} else unset($za_vi_settings[$is_forum ? 'flr' : 'tlr'][$id]);

			// Save new lock users			
			$list = za_vi_explode_user_list($_POST['za_vi_locked_users']);

			if(!empty($list)) $za_vi_settings[$is_forum ? 'flu' : 'tlu'][$id] = $list;
			else unset($za_vi_settings[$is_forum ? 'flu' : 'tlu'][$id]);

			// Save new lock exception users
			$list = za_vi_explode_user_list($_POST['za_vi_ex_locked_users']);

			if(!empty($list)) $za_vi_settings[$is_forum ? 'felu' : 'telu'][$id] = $list;
			else unset($za_vi_settings[$is_forum ? 'felu' : 'telu'][$id]);
			...
			if($is_forum)
			{
				...
				} else
				{
					...
					if(get_total_forums() == 1 && (!empty($za_vi_settings['fhu'][$id]) || !empty($za_vi_settings['fhr'][$id])))
					{
						unset($za_vi_settings['fhu'][$id]);
						unset($za_vi_settings['fhr'][$id]);
